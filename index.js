...
function getCategoryName(categoryId) {
  return rp({
    ...
    if (results && results.categories) {
      const currentCategory = results.categories.find((category) => { return category.id === categoryId; });
...
async function shareToFBGroup(topic, delay =0){
  if(topic && topic.id && topic.archetype === 'regular'){ // regular가 개인 message 생성도 topic이어서 일반 topic인지 구분
    ...
    if(updatedTopic){ // Is it existed or not?
      const displayCategoryName = await getCategoryName(updatedTopic.category_id).catch(()=>{
      ...
      console.log(message);
      return gotoGroupAndPost(message).catch( e => {
        console.error(e);
